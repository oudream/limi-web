var apiContent = {
    ...
    openAPI: function() {
        if (apiContent.searchNodes.length > 0) {
            let setting_zTree = $.fn.zTree.getZTreeObj('settingTree'),
            ...
            let node = apiContent.searchNodes[apiContent.searchNodesCur];
    ...
    searchNode: function(e) {
        ...
        let value = $.trim(apiContent.curKey);
        ...
        let settingNodeList = setting_zTree.getNodesByFilter(apiContent.searchFilter);
        let functionNodeList = function_zTree.getNodesByFilter(apiContent.searchFilter);
        let treenodeNodeList = treenode_zTree.getNodesByFilter(apiContent.searchFilter);
    ...
    searchFilter: function(node) {
        let value = $.trim(apiContent.searchKey.get(0).value).toLowerCase();
    ...
    searchCur: function(init) {
        let result = apiContent.searchNodes;
        if (init) {
            apiContent.searchResultInput.removeClass('noResult').attr('value', '');
    ...
    updateNodes: function(highlight) {
        let setting_zTree = $.fn.zTree.getZTreeObj('settingTree'),
            ...
            function_zTree = $.fn.zTree.getZTreeObj('functionTree'),
            node = null;
        for ( let i=0, l=apiContent.searchNodes.length; i<l; i++) {
    ...
    overlayAutoClose: function(e) {
        let eId = e.target.id, eRel = e.target.getAttribute('rel'), eClass = e.target.className;
    ...
    overlayShow: function(target, isSameNode) {
        let w = $(window), o = apiContent.overlayDiv, a = apiContent.overlayArrow,
        ...
        let h = apiContent.tmpDiv.outerHeight({margin: true}) + apiContent.overlaySearch.outerHeight();
        if ((t + h) > wMaxTop) {
            t = wMaxTop - h;
        }
