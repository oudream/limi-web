...
(function($) {
    ...
    $.extend($.jgrid, {
        ...
        parseDate: function(format, date, newformat, opts) {
            let	token = /\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g,
				...
				// parsing for month names
                    for (k=0, hl=format.length; k<hl; k++) {
                        if (format[k] === 'M') {
                            ...
                            if (dM !== -1 && dM < 12) {
                            date[k] = dM+1; ts.m = date[k];
                        }
                        }
                        if (format[k] === 'F') {
                            dM = $.inArray(date[k], opts.monthNames, 12);
                            if (dM !== -1 && dM > 11) {
                            date[k] = dM+1-12; ts.m = date[k];
                        }
                        }
                        if (format[k] === 'a') {
        ...
        getXmlData: function(obj, expr, returnObj) {
            let ret, m = typeof expr === 'string' ? expr.match(/^(.*)\[(\w+)\]$/) : null;
            if (typeof expr === 'function') {
                return expr(obj);
            }
            if (m && m[2]) {
        ...
        cellWidth: function() {
            let $testDiv = $('<div class=\'ui-jqgrid\' style=\'left:10000px\'><table class=\'ui-jqgrid-btable\' style=\'width:5px;\'><tr class=\'jqgrow\'><td style=\'width:5px;display:block;\'></td></tr></table></div>'),
		...
		// License MIT http://jlinq.codeplex.com/license
            let QueryObject=function(d, q) {
                ...
                this._doSort=function(d, q) {
                    ...
                    q++;
                    let values=self._getGroup(d, by, dir, type, dfmt), results=[], i, j, sorted;
                ...
                this._getOrder=function(data, by, dir, type, dfmt, sfunc) {
                    let sortData=[], _sortData=[], newDir = dir==='a' ? 1 : -1, i, ab, j,
                        findSortKey;
                    ...
                    if (type === 'float' || type=== 'number' || type=== 'currency' || type=== 'numeric') {
                        findSortKey = function($cell) {
                            let key = parseFloat( String($cell).replace(_stripNum, ''));
        ...
        return this.each( function() {
            ...
            var ts= this, grid={
                    ...
                    dragStart: function(i, x, y) {
                        let gridLeftPos = $(this.bDiv).offset().left;
                    ...
                    dragMove: function(x) {
                        if (this.resizing) {
                            let diff = x.clientX-this.resizing.startX,
                    ...
                    populateVisible: function() {
                        ...
                        grid.timer = null;
                        let dh = $(grid.bDiv).height();
                        if (!dh) {
                            return;
                        }
                        let table = $('table:first', grid.bDiv);
                        let rows, rh;
                        ...
                        let rn = p.rowNum;
                        let scrollTop = grid.scrollTop = grid.bDiv.scrollTop;
                        let ttop = Math.round(table.position().top) - scrollTop;
                        let tbot = ttop + table.height();
                        let div = rh * rn;
                        let page, npage, empty;
                        if ( tbot < dh && ttop <= 0 &&
                    ...
                    scrollGrid: function( e ) {
                        if (p.scroll) {
                            let scrollTop = grid.bDiv.scrollTop;
            ...
            let gv = $('<div class=\'ui-jqgrid-view\'></div>'),
                isMSIE = $.jgrid.msie;
            ...
            let eg = $('<div class=\'ui-jqgrid ui-widget ui-widget-content ui-corner-all\'></div>');
            ...
            var sortkeys = ['shiftKey', 'altKey', 'ctrlKey'],
                ...
                formatter = function(rowId, cellval, colpos, rwdat, _act) {
                    let cm = ts.p.colModel[colpos], v;
                    if (cm.formatter !== undefined) {
                        rowId = String(ts.p.idPrefix) !== '' ? $.jgrid.stripPref(ts.p.idPrefix, rowId) : rowId;
                        let opts= {rowId: rowId, colModel: cm, gid: ts.p.id, pos: colpos};
                ...
                addMulti = function(rowid, pos, irow, checked) {
                    let	v = '<input role="checkbox" type="checkbox"'+' id="jqg_'+ts.p.id+'_'+rowid+'" class="cbox" name="jqg_'+ts.p.id+'_'+rowid+'"' + (checked ? 'checked="checked"' : '')+'/>',
                        prp = formatCol( pos, irow, '', null, rowid, true);
                    return '<td role="gridcell" '+prp+'>'+v+'</td>';
                },
                addRowNum = function(pos, irow, pG, rN) {
                    let v = (parseInt(pG, 10)-1)*parseInt(rN, 10)+1+irow,
                ...
                refreshIndex = function() {
                    let datalen = ts.p.data.length, idname, i, val;
                ...
                constructTr = function(id, hide, altClass, rd, cur, selected) {
                    let tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
                ...
                addXmlData = function(xml, t, rcnt, more, adjust) {
                    ...
                    let self= $(ts), i, fpos, ir=0, v, gi=ts.p.multiselect===true?1:0, si=0, addSubGridCell, ni=ts.p.rownumbers===true?1:0, idn, getId, f=[], F, rd ={}, xmlr, rid, rowData=[], cn=(ts.p.altRows === true) ? ts.p.altclass:'', cn1;
                    ...
                    let gxml = $.jgrid.getXmlData( xml, xmlRd.root, true);
                    gxml = $.jgrid.getXmlData( gxml, xmlRd.row, true);
                    if (!gxml) {
                        gxml = [];
                    }
                    let gl = gxml.length, j=0, grpdata=[], rn = parseInt(ts.p.rowNum, 10), br=ts.p.scroll?$.jgrid.randId():1, altr;
                    if (gl > 0 && ts.p.page <= 0) {
                        ts.p.page = 1;
                    }
                    if (gxml && gl) {
                        ...
                        while (j<gl) {
                            ...
                            cn1 = (altr+j)%2 === 1 ? cn : '';
                            let iStartTrTag = rowData.length;
                            rowData.push('');
                            if ( ni ) {
                ...
                addJSONData = function(data, t, rcnt, more, adjust) {
                    ...
                    let self = $(ts), ir=0, v, i, j, f=[], cur, gi=ts.p.multiselect?1:0, si=ts.p.subGrid===true?1:0, addSubGridCell, ni=ts.p.rownumbers===true?1:0, arrayReader=orderedCols(gi+si+ni), objectReader=reader(frd), rowReader, len, drows, idn, rd={}, fpos, idr, rowData=[], cn=(ts.p.altRows === true) ? ts.p.altclass:'', cn1;
                    ...
                    let rn = parseInt(ts.p.rowNum, 10), br=ts.p.scroll?$.jgrid.randId():1, altr, selected=false, selr;
                    if (adjust) {
                        rn *= adjust+1;
                    }
                    ...
                    let afterInsRow = $.isFunction(ts.p.afterInsertRow), grpdata=[], hiderow=false, groupingPrepare;
                    if (ts.p.grouping) {
                        hiderow = ts.p.groupingView.groupCollapse === true;
                        groupingPrepare = $.jgrid.getMethod('groupingPrepare');
                    }
                    while (i<len) {
						...
						// reread id by name
                            ...
                            if (idr === undefined) {
                                ...
                                if (f.length===0) {
                                    if (dReader.cell) {
                                let ccur = $.jgrid.getAccessor(cur, dReader.cell) || cur;
                        ...
                        while (ir<len && drows[ir]) {
							...
							// reread id by name
                                ...
                                if (idr === undefined) {
                                    ...
                                    if (f.length===0) {
                                if (dReader.cell) {
                                    let ccur2 = $.jgrid.getAccessor(cur, dReader.cell) || cur;
                ...
                addLocalData = function() {
                    let st = ts.p.multiSort ? [] : '', sto=[], fndsort=false, cmtypes={}, grtypes=[], grindexes=[], srcformat, sorttype, newformat;
                    if (!$.isArray(ts.p.data)) {
                        return;
                    }
                    let grpview = ts.p.grouping ? ts.p.groupingView : false, lengrp, gin;
                    ...
                    function tojLinq( group ) {
                        let s = 0, index, gor, ror, opr, rule;
                    ...
                    if (ts.p.search === true) {
                        let srules = ts.p.postData.filters;
                        if (srules) {
                    ...
                    if ((ts.p.search || ts.p.resetsearch) && ts.p.grouping && ts.p.groupingView._locgr) {
                        ...
                        let j, grPrepare = $.jgrid.getMethod('groupingPrepare'), key, udc;
                ...
                updatepager = function(rn, dnd) {
                    let cp, last, base, from, to, tot, fmt, pgboxes = '', sppg,
                    ...
                    if (ts.p.scroll) {
                        let rows = $('tbody:first > tr:gt(0)', ts.grid.bDiv);
                        base = to - rows.length;
                        ts.p.reccount = rows.length;
                        let rh = rows.outerHeight() || ts.grid.prevRowHeight;
                        if (rh) {
                            let top = base * rh;
                            let height = parseInt(ts.p.records, 10) * rh;
                ...
                populate = function(npage) {
                    if (!ts.grid.hDiv.loading) {
                        let pvis = ts.p.scroll && npage === false,
                        ...
                        let lcf = $.isFunction(ts.p.loadComplete), lc = lcf ? ts.p.loadComplete : null;
                        let adjust = 0;
                        npage = npage || 1;
                        if (npage > 1) {
                            if (pN.npage !== null) {
                        ...
                        if (ts.p.grouping) {
                            ...
                            let grp = ts.p.groupingView, gi, gs='';
                        ...
                        let rcnt = !ts.p.scroll ? 1 : ts.rows.length-1;
                        let bfr = $(ts).triggerHandler('jqGridBeforeRequest');
                        if (bfr === false || bfr === 'stop') {
                            return;
                        }
                        if ($.isFunction(ts.p.datatype)) {
                            ts.p.datatype.call(ts, ts.p.postData, 'load_'+ts.p.id, rcnt, npage, adjust); return;
                        }
                ...
                setHeadCheckBox = function( checked ) {
                    ...
                    let fid = ts.p.frozenColumns ? ts.p.id+'_frozen' : '';
                    if (fid) {
			...
			// TBD - consider escaping pgid with pgid = $.jgrid.jqID(pgid);
                    let sep = '<td class=\'ui-pg-button ui-state-disabled\' style=\'width:4px;\'><span class=\'ui-separator\'></span></td>',
                        pginp = '',
                        ...
                        clearVals = function(onpaging) {
                            ...
                            ts.p.selrow = null;
                            if (ts.p.multiselect) {
                                ts.p.selarrrow =[]; setHeadCheckBox( false );
                            }
                            ts.p.savedRow = [];
                            return true;
                        };
			...
			.attr('dir', 'ltr'); // explicit setting
                ...
                multiSort = function(iCol, obj ) {
                    let splas, sort='', cm = ts.p.colModel, fs=false, ls,
                ...
                sortData = function(index, idxcol, reload, sor, obj) {
                    ...
                    if (ts.p.multiSort) {
                        ...
                        let previousSelectedTh = ts.grid.headers[ts.p.lastsort].el, newSelectedTh = ts.p.frozenColumns ? obj : ts.grid.headers[idxcol].el;
                    ...
                    if (ts.p.scroll) {
                        let sscroll = ts.grid.bDiv.scrollLeft;
                ...
                setColWidth = function() {
                    let initwidth = 0, brd=$.jgrid.cell_width? 0: intNum(ts.p.cellLayout, 0), vc=0, lvc, scw=intNum(ts.p.scrollOffset, 0), cw, hs=false, aw, gw=0, cr;
                    $.each(ts.p.colModel, function() {
                        if (this.hidden === undefined) {
                            this.hidden=false;
                        }
                        if (ts.p.grouping && ts.p.autowidth) {
                            let ind = $.inArray(this.name, ts.p.groupingView.groupField);
                ...
                getOffset = function(iCol) {
                    let $th = $(ts.grid.headers[iCol].el), ret = [$th.position().left + $th.outerWidth()];
                ...
                getColumnHeaderIndex = function(th) {
                    let i, headers = ts.grid.headers, ci = $.jgrid.getCellIndex(th);
            ...
            let thead = '<thead><tr class=\'ui-jqgrid-labels\' role=\'rowheader\'>',
                tdc, idn, w, res, sort,
            ...
            for (i=0; i<this.p.colNames.length; i++) {
                let tooltip = ts.p.headertitles ? (' title="'+$.jgrid.stripHtml(ts.p.colNames[i])+'"') :'';
            ...
            if (this.p.multiselect) {
                ...
                $('#cb_'+$.jgrid.jqID(ts.p.id), this).bind('click', function() {
                    ts.p.selarrrow = [];
                    let froz = ts.p.frozenColumns === true ? ts.p.id + '_frozen' : '';
                    if (this.checked) {
            ...
            $('th', thr).each(function( j ) {
                ...
                let ci = getColumnHeaderIndex(this);
                ...
                let s = 'th>div.ui-jqgrid-sortable', r, d;
                if (!ts.p.viewsortcols[2]) {
                ...
                let ci;
                if (ts.p.frozenColumns) {
                    let tid = $(this)[0].id.substring( ts.p.id.length + 1 );
            ...
            firstr = null;
            let hTable = $('<table class=\'ui-jqgrid-htable\' style=\'width:'+ts.p.tblwidth+'px\' role=\'grid\' aria-labelledby=\'gbox_'+this.id+'\' cellspacing=\'0\' cellpadding=\'0\' border=\'0\'></table>').append(thead),
            ...
            $(ts).before(grid.hDiv).click(function(e) {
                    ...
                    if (ts.p.cellEdit === true) {
                        ...
                        if (ts.p.multiselect && ts.p.multiboxonly) {
                            if (scb) {
                                ...
                                let frz = ts.p.frozenColumns ? ts.p.id+'_frozen' : '';
                                $(ts.p.selarrrow).each(function(i, n) {
                            let trid = $(ts).jqGrid('getGridRowById', n);
                            if (trid) {
                ...
                if (ts.p.datatype==='local') {
                    $(ts).jqGrid('resetSelection'); if (ts.p.data.length) {
                        refreshIndex();
                    }
    ...
    arf.removeClass('ui-state-hover');
		...
		.addClass('ui-jqgrid-titlebar ui-jqgrid-caption'+(dir==='rtl' ? '-rtl' :'' )+' ui-widget-header ui-corner-top ui-helper-clearfix');
            ...
            hb = null;
            if (ts.p.caption) {
                let tdt = ts.p.datatype;
                if (ts.p.hidegrid===true) {
                    $('.ui-jqgrid-titlebar-close', grid.cDiv).click( function(e) {
                        let onHdCl = $.isFunction(ts.p.onHeaderClick),
    ...
    $.jgrid.extend({
        ...
        setSelection: function(selection, onsr, e) {
            return this.each(function() {
                let $t = this, stat, pt, ner, ia, tpsr, fid, csr;
                ...
                function scrGrid(iR) {
                    let ch = $($t.grid.bDiv)[0].clientHeight,
        ...
        getRowData: function( rowid ) {
            let res = {}, resall, getall=false, len, j=0;
            this.each(function() {
                let $t = this, nm, ind;
                if (rowid === undefined) {
                    getall = true;
        ...
        delRowData: function(rowid) {
            let success = false, rowInd, ia, nextRow;
            this.each(function() {
                let $t = this;
                ...
                if ($t.p.datatype === 'local') {
                    let id = $.jgrid.stripPref($t.p.idPrefix, rowid),
                        pos = $t.p._index[id];
                    if (pos !== undefined) {
        ...
        setRowData: function(rowid, data, cssp) {
            let nm, success=true, title;
            this.each(function() {
                if (!this.grid) {
                    return false;
                }
                let t = this, vl, ind, cp = typeof cssp, lcdata={};
                ...
                if ( data ) {
                    try {
                        $(this.p.colModel).each(function(i) {
                            nm = this.name;
                            let dval =$.jgrid.getAccessor(data, nm);
                            if ( dval !== undefined) {
        ...
        addRowData: function(rowid, rdata, pos, src) {
            ...
            let success = false, nm, row, gi, si, ni, sind, i, v, prp='', aradd, cnm, cn, data, cm, id;
            if (rdata) {
                if ($.isArray(rdata)) {
                    aradd=true;
                    pos = 'last';
                ...
                this.each(function() {
                    let t = this, datalen = rdata.length;
        ...
        footerData: function(action, data, format) {
            let nm, success=false, res={}, title;
        ...
        showHideCol: function(colname, show) {
            return this.each(function() {
                let $t = this, fndh=false, brd=$.jgrid.cell_width ? 0: $t.p.cellLayout, cw;
                if (!$t.grid ) {
                    return;
                }
            ...
            return this.each(function() {
                ...
                if (shrink===true) {
                    ...
                    if ($t.p.tblwidth > nwidth) {
                        let delta = $t.p.tblwidth - parseInt(nwidth, 10);
                        $t.p.tblwidth = nwidth;
        ...
        setLabel: function(colname, nData, prop, attrp ) {
            return this.each(function() {
                ...
                if (pos>=0) {
                    let thecol = $('tr.ui-jqgrid-labels th:eq('+pos+')', $t.grid.hDiv);
                    if (nData) {
                        let ico = $('.s-ico', thecol);
        ...
        setCell: function(rowid, colname, nData, cssp, attrp, forceupd) {
            return this.each(function() {
                ...
                if (pos>=0) {
                    let ind = $($t).jqGrid('getGridRowById', rowid);
                    if (ind) {
                        let tcell = $('td:eq('+pos+')', ind);
        ...
        getCol: function(col, obj, mathopr) {
            ...
            this.each(function() {
                ...
                if (pos>=0) {
                    let ln = $t.rows.length, i =0, dlen=0;
                    if (ln && ln>0) {
                        while (i<ln) {
        ...
        clearGridData: function(clearfooter) {
            return this.each(function() {
                ...
                if ($t.p.deepempty) {
                    ...
                    let trf = $('#'+$.jgrid.jqID($t.p.id)+' tbody:first tr:first')[0];
        ...
        bindKeys: function( settings ) {
            ...
            return this.each(function() {
                ...
                $($t).keydown(function(event) {
                    let target = $($t).find('tr[tabindex=0]')[0], id, r, mind,
                        expanded = $t.p.treeReader.expanded_field;
				// check.json for arrow keys
                    if (target) {
					...
					// check.json if enter was pressed on a grid or treegrid node
...
**/
    ...
    $.jgrid.extend({
        ...
        sortGrid: function(colname, reload, sor) {
            return this.each(function() {
                ...
                if ( idx !== -1 ) {
                    let sort = $t.p.colModel[idx].sortable;
                    if (!sobj) {
        ...
        GridUnload: function() {
            return this.each(function() {
                ...
                let defgrid = {id: $(this).attr('id'), cl: $(this).attr('class')};
                if (this.p.pager) {
        ...
        filterToolbar: function(p) {
            ...
            return this.each(function() {
                ...
                let triggerToolbar = function() {
                        let sdata={}, j=0, v, nm, sopt={}, so;
                        $.each($t.p.colModel, function() {
                            let $elem = $('#gs_'+$.jgrid.jqID(this.name), (this.frozen===true && $t.p.frozenColumns === true) ? $t.grid.fhDiv : $t.grid.hDiv);
                        ...
                        let bsr = $($t).triggerHandler('jqGridToolbarBeforeSearch') === 'stop' ? true : false;
                    ...
                    clearToolbar = function(trigger) {
                        ...
                        $.each($t.p.colModel, function() {
                            let v, $elem = $('#gs_'+$.jgrid.jqID(this.name), (this.frozen===true && $t.p.frozenColumns === true) ? $t.grid.fhDiv : $t.grid.hDiv);
                        ...
                        let bcv = $($t).triggerHandler('jqGridToolbarBeforeClear') === 'stop' ? true : false;
                    ...
                    toggleToolbar = function() {
                        let trow = $('tr.ui-search-toolbar', $t.grid.hDiv),
                    ...
                    buildRuleMenu = function( elem, left, top ) {
                        ...
                        let fs = $('.ui-jqgrid-view').css('font-size') || '11px';
                        let str = '<ul id="sopt_menu" class="ui-search-menu" role="menu" tabindex="0" style="font-size:'+fs+';left:'+left+'px;top:'+top+'px;">',
                            selected = $(elem).attr('soper'), selclass,
                            aoprs = [], ina;
                        let i=0, nm =$(elem).attr('colname'), len = $t.p.colModel.length;
                        while (i<len) {
                        ...
                        let cm = $t.p.colModel[i], options = $.extend({}, cm.searchoptions);
                        if (!options.sopt) {
                            options.sopt = [];
    ...
    if (p.autosearch===true) {
        let inpelm = $(elem).parent().next().children()[0];
        if ( $(inpelm).val() || v==='nu' || v ==='nn') {
			...
			// create the row
                let tr = $('<tr class=\'ui-search-toolbar\' role=\'rowheader\'></tr>');
                let timeoutHnd;
                $.each($t.p.colModel, function(ci) {
                    let cm=this, soptions, surl, self, select = '', sot='=', so, i,
                    ...
                    if (this.search) {
                        if (p.searchOperators) {
                            ...
                            let st = soptions.searchtitle != null ? soptions.searchtitle : p.operandTitle;
                        ...
                        if (soptions.clearSearch) {
                            let csv = p.resetTitle || 'Clear Search Value';
                        ...
                        case 'select':
							...
							// primitive jQuery load
                                ...
                                $.ajax($.extend({
                                    ...
                                    let d = soptions.buildSelect(res);
                                    if (d) {
                        ...
                        case 'custom':
                            ...
                            try {
                                if ($.isFunction(soptions.custom_element)) {
                                    let celm = soptions.custom_element.call($t, soptions.defaultValue !== undefined ? soptions.defaultValue: '', soptions);
                                    if (celm) {
        ...
        destroyGroupHeader: function(nullHeader)	{
            ...
            return this.each(function()		{
                let $t = this, $tr, i, l, headers, $th, $resizing, grid = $t.grid,
			...
			// startColumnName,
                ...
                let $firstRow,
                    inColumnHeader = function(text, columnHeaders) {
                        let length = columnHeaders.length, i;
                        for (i = 0; i < length; i++) {
        ...
        setFrozenColumns: function() {
            return this.each(function() {
                ...
                let $t = this, cm = $t.p.colModel, i=0, len = cm.length, maxfrozen = -1, frozen= false;
			...
			// get the max index of frozen col
                ...
                if ( maxfrozen>=0 && frozen) {
                    let top = $t.p.caption ? $($t.grid.cDiv).outerHeight() : 0,
				...
				// headers
                    ...
                    let htbl = $('.ui-jqgrid-htable', '#gview_'+$.jgrid.jqID($t.p.id)).clone(true);
				// groupheader support - only if useColSpanstyle is false
                    if ($t.p.groupHeader) {
                        $('tr.jqg-first-row-header, tr.jqg-third-row-header', htbl).each(function() {
                            $('th:gt('+maxfrozen+')', this).remove();
                        });
                        let swapfroz = -1, fdel = -1, cs, rs;
				...
				.mousemove(function(e) {
    if ($t.grid.resizing) {
        $t.grid.dragMove(e); return false;
    }
});
                    $($t).bind('jqGridResizeStop.setFrozenColumns', function(e, w, index) {
                        let rhth = $('.ui-jqgrid-htable', $t.grid.fhDiv);
                        $('th:eq('+index+')', rhth).width( w );
                        let btd = $('.ui-jqgrid-btable', $t.grid.fbDiv);
                        $('tr:first td:eq('+index+')', btd).width( w );
                    });
				// sorting stuff
                    $($t).bind('jqGridSortCol.setFrozenColumns', function(e, index, idxcol) {
                        let previousSelectedTh = $('tr.ui-jqgrid-labels:last th:eq('+$t.p.lastsort+')', $t.grid.fhDiv), newSelectedTh = $('tr.ui-jqgrid-labels:last th:eq('+idxcol+')', $t.grid.fhDiv);
				...
				// TODO support for setRowData
                    ...
                    $($t).bind('jqGridAfterGridComplete.setFrozenColumns', function() {
                        ...
                        let btbl = $('#'+$.jgrid.jqID($t.p.id)).clone(true);
                        ...
                        if ($t.p.hoverrows === true) {
							...
							function() {
    $(this).addClass('ui-state-hover'); $('#'+$.jgrid.jqID(this.id), '#'+$.jgrid.jqID($t.p.id)).addClass('ui-state-hover');
},
							function() {
    $(this).removeClass('ui-state-hover'); $('#'+$.jgrid.jqID(this.id), '#'+$.jgrid.jqID($t.p.id)).removeClass('ui-state-hover');
}
						);
                            $('tr.jqgrow', '#'+$.jgrid.jqID($t.p.id)).hover(
							function() {
    $(this).addClass('ui-state-hover'); $('#'+$.jgrid.jqID(this.id), '#'+$.jgrid.jqID($t.p.id)+'_frozen').addClass('ui-state-hover');
},
							function() {
    $(this).removeClass('ui-state-hover'); $('#'+$.jgrid.jqID(this.id), '#'+$.jgrid.jqID($t.p.id)+'_frozen').removeClass('ui-state-hover');
}
						);
                        }
                        btbl=null;
...
(function($) {
    ...
    $.jqm = {
        hash: {},
        open: function(s, t) {
            let h=H[s], c=h.c, cc='.'+c.closeClass, z=(parseInt(h.w.css('z-index'))); z=(z>0)?z:3000; let o=$('<div></div>').css({'height': '100%', 'width': '100%', 'position': 'fixed', 'left': 0, 'top': 0, 'z-index': z-1, 'opacity': c.overlay/100}); if (h.a) return F; h.t=t; h.a=true; h.w.css('z-index', z);
            if (c.modal) {
                if (!A[0]) {
                    setTimeout(function() {
                        L('bind');
            ...
            if (c.ajax) {
                let r=c.target||h.w, u=c.ajax; r=(typeof r == 'string')?$(r, h.w):$(r); u=(u.substr(0, 1) == '@')?$(t).attr(u.substring(1)):u;
        ...
        close: function(s) {
            let h=H[s]; if (!h.a) return F; h.a=F;
            if (A[0]) {
    ...
    var s=0, H=$.jqm.hash, A=[], F=false,
        ...
        m=function(e) {
            let h=H[A[A.length-1]], r=(!$(e.target).parents('.jqmID'+h.s)[0]); if (r) {
                $('.jqmID'+h.s).each(function() {
                    let $self=$(this), offset=$self.offset(); if (offset.top<=e.pageY && e.pageY<=offset.top+$self.height() && offset.left<=e.pageX && e.pageX<=offset.left+$self.width()) {
        ...
        hs=function(w, t, c) {
            return w.each(function() {
                let s=this._jqm; $(t).each(function() {
                    if (!this[c]) {
                        this[c]=[]; $(this).click(function() {
                            for (let i in {jqmShow: 1, jqmHide: 1}) for (let s in this[i]) if (H[this[i][s]])H[this[i][s]].w[i](this); return F;
                        });
                    } this[c].push(s);
                });
            });
        ...
        i=function(e, h, k, aR) {
            return e.each(function() {
                ...
                h.bind('mousedown', {e: e, k: k}, function(v) {
                    let d=v.data, p={}; E=d.e; E1 = aR ? $(aR) : false;
...
let xmlJsonClass = {
    ...
    xml2json: function(xml, tab) {
        ...
        let nws = this.removeWhite(xml);
        let obj = this.toObj(nws);
        let json = this.toJson(obj, xml.nodeName, '\t');
            ...
            if (v instanceof Array) {
                if (v.length === 0) {
                    ...
                    for (i = 0, n = v.length; i < n; i += 1) {
                        let sXml = ind + toXml(v[i], name, ind+'\t') + '\n';
                        xml += sXml;
                    }
                }
    ...
    toJson: function(o, name, ind, wellform) {
        ...
        let json = name ? ('"' + name + '"') : '', tab = '\t', newline = '\n';
        if (!wellform) {
            tab= ''; newline= '';
        }
...
(function($) {
    ...
    $.fmatter.util = {
        ...
        NumberFormat: function(nData, opts) {
            ...
            if ($.fmatter.isNumber(nData)) {
                let bNegative = (nData < 0);
                let sOutput = String(nData);
                let sDecimalSeparator = opts.decimalSeparator || '.';
                let nDotIndex;
                if ($.fmatter.isNumber(opts.decimalPlaces)) {
					// Round to the correct decimal place
                    let nDecimalPlaces = opts.decimalPlaces;
                    let nDecimal = Math.pow(10, nDecimalPlaces);
                ...
                if (opts.thousandsSeparator) {
                    let sThousandsSeparator = opts.thousandsSeparator;
    ...
    $.fn.fmatter.checkbox =function(cval, opts) {
        let op = $.extend({}, opts.checkbox), ds;
        ...
        let bchk = cval.search(/(false|f|0|no|n|off|undefined)/i)<0 ? ' checked=\'checked\' ' : '';
        return '<input type="checkbox" ' + bchk + ' value="'+ cval+'" offval="no" '+ds+ '/>';
    };
    $.fn.fmatter.link = function(cellval, opts) {
        let op = {target: opts.target};
        let target = '';
        if (opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
            op = $.extend({}, op, opts.colModel.formatoptions);
        }
    ...
    $.fn.fmatter.showlink = function(cellval, opts) {
        let op = {baseLinkUrl: opts.baseLinkUrl, showAction: opts.showAction, addParam: opts.addParam || '', target: opts.target, idName: opts.idName},
            target = '', idUrl;
        if (opts.colModel !== undefined && opts.colModel.formatoptions !== undefined) {
            op = $.extend({}, op, opts.colModel.formatoptions);
        }
    ...
    $.fn.fmatter.select = function(cellval, opts) {
        ...
        let oSelect = false, ret=[], sep, delim;
        ...
        if (oSelect) {
            let	msl = (opts.colModel.editoptions != null && opts.colModel.editoptions.multiple === true) === true ? true : false,
                scell = [], sv;
            if (msl) {
                scell = cellval.split(','); scell = $.map(scell, function(n) {
                    return $.trim(n);
                });
            }
            if ($.fmatter.isString(oSelect)) {
				// mybe here we can use some caching with care ????
                let so = oSelect.split(delim), j=0, i;
    ...
    $.fn.fmatter.actions = function(cellval, opts) {
        let op={keys: false, editbutton: true, delbutton: true, editformbutton: false},
    ...
    $.unformat = function(cellval, options, pos, cnt) {
        ...
        let ret, formatType = options.colModel.formatter,
        ...
        if (unformatFunc !== undefined && $.isFunction(unformatFunc) ) {
            ...
            let opts = $.jgrid.formatter || {}, stripTag;
            switch (formatType) {
            case 'integer' :
    ...
    $.unformat.select = function(cellval, options, pos, cnt) {
        ...
        let ret = [];
        let cell = $(cellval).text();
        if (cnt===true) {
            return cell;
        }
        let op = $.extend({}, options.colModel.formatoptions !== undefined ? options.colModel.formatoptions: options.colModel.editoptions),
            sep = op.separator === undefined ? ':' : op.separator,
            delim = op.delimiter === undefined ? ';' : op.delimiter;

        if (op.value) {
            let oSelect = op.value,
                msl = op.multiple === true ? true : false,
                scell = [], sv;
            if (msl) {
                scell = cell.split(','); scell = $.map(scell, function(n) {
                    return $.trim(n);
                });
            }
            if ($.fmatter.isString(oSelect)) {
...
// Modal functions
        ...
        hideModal: function(selector, o) {
            ...
            if (o.onClose) {
                let oncret = o.gb && typeof o.gb === 'string' && o.gb.substr(0, 6) === '#gbox_' ? o.onClose.call($('#' + o.gb.substr(6))[0], selector) : o.onClose(selector);
...
// Helper functions
        ...
        createModal: function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
            ...
            let mw = document.createElement('div'), rtlsup, self = this;
            css = $.extend({}, css || {});
    ...
    ahr.removeClass('ui-state-hover');
		...
		.append('<span class=\'ui-icon ui-icon-closethick\'></span>');
            ...
            $('a.ui-jqdialog-titlebar-close', mh).click(function() {
                let oncm = $('#'+$.jgrid.jqID(aIDs.themodal)).data('onClose') || p.onClose;
                let gboxclose = $('#'+$.jgrid.jqID(aIDs.themodal)).data('gbox') || p.gbox;
            ...
            if (!p.zIndex) {
                let parentZ = $(insertSelector).parents('*[role=dialog]').filter(':first').css('z-index');
                if (parentZ) {
		...
		.attr({'tabIndex': '-1', 'role': 'dialog', 'aria-labelledby': aIDs.modalhead, 'aria-hidden': 'true'});
            ...
            if (p.closeOnEscape === true) {
                $(mw).keydown( function( e ) {
                    if ( e.which == 27 ) {
                        let cone = $('#'+$.jgrid.jqID(aIDs.themodal)).data('onClose') || p.onClose;
		...
		// in case there is no jqModal
            ...
            let dh = isNaN(mopt.dataheight) ? mopt.dataheight : mopt.dataheight+'px',
                cn = 'text-align:'+mopt.align+';';
            let cnt = '<div id=\'info_id\'>';
            ...
            if (buttstr) {
                $.each(mopt.buttons, function(i) {
                    $('#'+$.jgrid.jqID(this.id), '#info_id').bind('click', function() {
                        mopt.buttons[i].onClick.call($('#info_dialog')); return false;
                    });
                });
            }
...
// Form Functions
        createEl: function(eltype, options, vl, autowidth, ajaxso) {
            ...
            function setAttributes(elm, atr, exl ) {
                let exclude = ['dataInit', 'dataEvents', 'dataUrl', 'buildSelect', 'sopt', 'searchhidden', 'defaultValue', 'attr', 'custom_element', 'custom_value'];
            ...
            case 'select' :
                ...
                if (options.dataUrl !== undefined) {
                    let rowid = null, postData = options.postData || ajaxso.postData;
                    try {
                        rowid = options.rowId;
                    } catch (e) {}
                    ...
                    $.ajax($.extend({
                        ...
                        success: function(data) {
                            let ovm = [], elem = this.elem, vl = this.vl,
            ...
            case 'custom' :
                ...
                try {
                    if ($.isFunction(options.custom_element)) {
                        let celm = options.custom_element.call($t, vl, options);
                        if (celm) {
...
// Date Validation Javascript
        checkDate: function(format, date) {
            let daysInFebruary = function(year) {
		...
		// we search for /,-,. for the date separator
            ...
            let j=-1, yln, dln=-1, mln=-1, i;
            for (i=0; i<format.length; i++) {
                let dv = isNaN(date[i]) ? 0 : parseInt(date[i], 10);
                tsp[format[i]] = dv;
                yln = format[i];
                if (yln.indexOf('y') !== -1) {
                    j=i;
	...
	// checks only hh:ss (and optional am/pm)
        ...
        checkValues: function(val, valref, customobject, nam) {
            let edtrul, i, nm, dft, len, g = this, cm = g.p.colModel;
			...
			// force required
                let rqfield = edtrul.required === false ? false : true;
                ...
                if (edtrul.custom === true) {
                    if ( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
                        if ($.isFunction(edtrul.custom_func)) {
                            let ret = edtrul.custom_func.call(g, val, nm, valref);
...
(function($) {
    ...
    $.fn.jqFilter = function( arg ) {
		...
		// setup filter in case if they is not defined
            ...
            let i, len = this.p.columns.length, cl,
		 ...
		 *
		*/
            let checkData = function(val, colModelItem) {
                let ret = [true, ''], $t = getGrid();
		...
		*/
		 ...
		 */
            this.reDraw = function() {
                ...
                let t = this.createTableForGroup(p.filter, null);
                $(this).append(t);
		 ...
		 */
			...
			// this table will hold all the group (tables) and rules (rows)
                let table = $('<table class=\'group ui-widget ui-widget-content\' style=\'border:0px none;\'><tbody></tbody></table>'),
			...
			// creating subgroup "+ {}", creating rule "+" or deleting the group "-"
                let th = $('<th colspan=\'5\' align=\''+align+'\'></th>');
                tr.append(th);

                if (this.p.ruleButtons === true) {
			// dropdown for: choosing group operator type
                    let groupOpSelect = $('<select class=\'opsel\'></select>');
    ...
    that.onchange(); // signals that the filter has changed
			...
			// button for adding a new subgroup
                let inputAddSubgroup ='<span></span>';
			...
			// button for adding a new rule
                    let inputAddRule = $('<input type=\'button\' value=\'+\' title=\'Add rule\' class=\'add-rule ui-add\'/>'), cm;
				...
				// but show only serchable and serchhidden = true fields
                            let searchable = (that.p.columns[i].search === undefined) ? true: that.p.columns[i].search,
			...
			// button for delete the group
                if (parentgroup !== null) { // ignore the first group
                    let inputDeleteGroup = $('<input type=\'button\' value=\'-\' title=\'Delete group\' class=\'delete-group\'/>');
			...
			// append subgroup rows
                if (group.groups !== undefined) {
                    for (i = 0; i < group.groups.length; i++) {
                        let trHolderForSubgroup = $('<tr></tr>');
                        table.append(trHolderForSubgroup);

                        let tdFirstHolderForSubgroup = $('<td class=\'first\'></td>');
                        trHolderForSubgroup.append(tdFirstHolderForSubgroup);

                        let tdMainHolderForSubgroup = $('<td colspan=\'4\'></td>');
		 ...
		 */
			...
			// be referenced in anonimous method calls
                ...
                let that=this, $t = getGrid(), tr = $('<tr></tr>'),
			...
			// create field container
                let ruleFieldTd = $('<td class=\'columns\'></td>');
                tr.append(ruleFieldTd);


			// dropdown for: choosing field
                let ruleFieldSelect = $('<select></select>'), ina, aoprs = [];
                ...
                ruleFieldSelect.bind('change', function() {
                    ...
                    let elm = $.jgrid.createEl.call($t, cm.inputtype, cm.searchoptions, '', true, that.p.ajaxSelectOptions || {}, true);
				...
				// operators
                    ...
                    if ( $.jgrid.msie && $.jgrid.msiever() < 9) {
                        let sw = parseInt($('select.selectopts', trpar)[0].offsetWidth, 10) + 1;
			...
			// create operator container
                let ruleOperatorTd = $('<td class=\'operators\'></td>');
			...
			// var RD = that.createElement(rule, rule.data);
                ...
                let ruleDataInput = $.jgrid.createEl.call($t, cm.inputtype, cm.searchoptions, rule.data, true, that.p.ajaxSelectOptions || {}, true);
                if (rule.op === 'nu' || rule.op === 'nn') {
                    $(ruleDataInput).attr('readonly', 'true');
                    $(ruleDataInput).attr('disabled', 'true');
                } // retain the state of disabled text fields in case of null ops
			// dropdown for: choosing operator
                let ruleOperatorSelect = $('<select class=\'selectopts\'></select>');
                ruleOperatorTd.append(ruleOperatorSelect);
                ruleOperatorSelect.bind('change', function() {
                    rule.op = $(ruleOperatorSelect).val();
                    trpar = $(this).parents('tr:first');
                    let rd = $('.input-elm', trpar)[0];
			...
			// create data container
                let ruleDataTd = $('<td class=\'data\'></td>');
    ...
    that.onchange(); // signals that the filter has changed
			...
			// create action container
                let ruleDeleteTd = $('<td></td>');
                tr.append(ruleDeleteTd);

			// create button for: delete rule
                if (this.p.ruleButtons === true) {
                    let ruleDeleteInput = $('<input type=\'button\' value=\'-\' title=\'Delete rule\' class=\'delete-rule ui-del\'/>');
...
**/
        ...
        searchGrid: function(p) {
            ...
            return this.each(function() {
                ...
                if ( $('#'+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
                    ...
                    let fil = $('<div><div id=\''+fid+'\' class=\'searchFilter\' style=\'overflow:auto\'></div></div>').insertBefore('#gview_'+$.jgrid.jqID($t.p.id)),
                        align = 'left', butleft ='';
                    ...
                    let columns = $.extend([], $t.p.colModel),
                    ...
                    if (!p.columns.length) {
						...
						// find first searchable column and set it if no default filter
                            if (!found) {
                                let searchable = (n.search === undefined) ? true: n.search,
            ...
            return this.each(function() {
                ...
                function getFormData() {
                    $(frmtb+' > tbody > tr > td > .FormElement').each(function() {
                        let celm = $('.customelement', this);
                        if (celm.length) {
                            let elem = celm[0], nm = $(elem).attr('name');
                            $.each($t.p.colModel, function() {
    ...
    selectedText[i] = $(selected).text();
                ...
                function createData(rowid, obj, tb, maxcols) {
                    let nm, hc, trdata, cnt=0, tmp, dc, elc, retpos=[], ind=false,
					...
					// hidden fields are included in the form
                        ...
                        if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
                            ...
                            let opt = $.extend({}, this.editoptions || {}, {id: nm, name: nm, rowId: rowid}),
						...
						// if(tmp === "" && this.edittype == "select") {tmp = $("option:eq(0)",elc).text();}
                            ...
                            if (frmopt.rowabove) {
                                let newdata = $('<tr><td class=\'contentinfo\' colspan=\''+(maxcols*2)+'\'>'+frmopt.rowcontent+'</td></tr>');
                                $(tb).append(newdata);
                                newdata[0].rp = rp;
                            }
                            if ( trdata.length===0 ) {
                    ...
                    if ( cnt > 0) {
                        let idrow = $('<tr class=\'FormData\' style=\'display:none\'><td class=\'CaptionTD\'></td><td colspan=\''+ (maxcols*2-1)+'\' class=\'DataTD\'><input class=\'FormElement\' id=\'id_g\' type=\'text\' name=\''+obj.p.id+'_id\' value=\''+rowid+'\'/></td></tr>');
                        idrow[0].rp = cnt+999;
                        $(tb).append(idrow);
                        if (rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
                ...
                function fillData(rowid, obj, fmid) {
                    let nm, cnt=0, tmp, fld, opt, vl, vlc;
                    if (rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
                        rp_ge[$t.p.id]._savedData = {}; rp_ge[$t.p.id]._savedData[obj.p.id+'_id']=rowid;
                    }
                    let cm = obj.p.colModel;
                    if (rowid === '_empty') {
                        $(cm).each(function() {
                            nm = this.name;
                    ...
                    let tre = $(obj).jqGrid('getInd', rowid, true);
                    if (!tre) {
                        return;
                    }
                        ...
                        if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && cm[i].editable===true) {
                            ...
                            case 'checkbox':
                                ...
                                if (cm[i].editoptions && cm[i].editoptions.value) {
                                    let cb = cm[i].editoptions.value.split(':');
                                    if (cb[0] === tmp) {
                ...
                function postIt() {
                    let copydata, ret=[true, '', ''], onCS = {}, opers = $t.p.prmNames, idname, oper, key, selr, i;

                    let retvals = $($t).triggerHandler('jqGridAddEditBeforeCheckValues', [$('#'+frmgr), frmoper]);
					...
					// we add to pos data array the action - the name is oper
						...
						// check.json to see if we have allredy this field in the form and if yes lieve it
                        ...
                        if ($t.p.treeGrid === true) {
                            if (postdata[oper] === opers.addoper) {
                                ...
                                let tr_par_id = $t.p.treeGridModel === 'adjacency' ? $t.p.treeReader.parent_id_field : 'parent_id';
                                postdata[tr_par_id] = selr;
                            }
                            for (i in $t.p.treeReader) {
                                if ($t.p.treeReader.hasOwnProperty(i)) {
                                    let itm = $t.p.treeReader[i];
                        ...
                        if (ret[0]) {
                            if (rp_ge[$t.p.id].useDataProxy) {
                                let dpret = $t.p.dataProxy.call($t, ajaxOptions, 'set_'+$t.p.id);
                                if (dpret === undefined) {
                                    dpret = [true, ''];
                                }
                                if (dpret[0] === false ) {
                ...
                function getCurrPos() {
                    let rowsInGrid = $($t).jqGrid('getDataIDs'),
                        selrow = $('#id_g', frmtb).val(),
                        pos = $.inArray(selrow, rowsInGrid);
                    return [pos, rowsInGrid];
                }

                let dh = isNaN(rp_ge[$(this)[0].p.id].dataheight) ? rp_ge[$(this)[0].p.id].dataheight : rp_ge[$(this)[0].p.id].dataheight+'px',
                ...
                $($t.p.colModel).each( function() {
                    let fmto = this.formoptions;
                    maxCols = Math.max(maxCols, fmto ? fmto.colpos || 0 : 0 );
                    maxRows = Math.max(maxRows, fmto ? fmto.rowpos || 0 : 0 );
                });
                $(frm).append(tbl);
                let flr = $('<tr id=\'FormError\' style=\'display:none\'><td class=\'ui-state-error\' colspan=\''+(maxCols*2)+'\'></td></tr>');
                flr[0].rp = 0;
                $(tbl).append(flr);
			// topinfo
			...
			// create data
                let rtlb = $t.p.direction === 'rtl' ? true :false,
                    bp = rtlb ? 'nData' : 'pData',
                    bn = rtlb ? 'pData' : 'nData';
                createData(rowid, $t, tbl, maxCols);
			// buttons at footer
                let bP = '<a id=\''+bp+'\' class=\'fm-button ui-state-default ui-corner-left\'><span class=\'ui-icon ui-icon-triangle-1-w\'></span></a>',
                ...
                let bt = '<table border=\'0\' cellspacing=\'0\' cellpadding=\'0\' class=\'EditTable\' id=\''+frmtborg+'_2\'><tbody><tr><td colspan=\'2\'><hr class=\'ui-widget-content\' style=\'margin:1px\'/></td></tr><tr id=\'Act_Buttons\'><td class=\'navButton\'>'+(rtlb ? bN+bP : bP+bN)+'</td><td class=\'EditButton\'>'+bS+bC+'</td></tr>';
                ...
                let tms = $('<div></div>').append(frm).append(bt);
    ...
    return false;
			...
			// here initform - only once
                ...
                $.jgrid.viewModal('#'+$.jgrid.jqID(IDs.themodal), {
                    ...
                    onHide: function(h) {
                        let fh = $('#editmod'+gID)[0].style.height;
                        if (fh.indexOf('px') > -1 ) {
                            fh = parseFloat(fh);
                        }
                        $($t).data('formProp', {
    ...
    $(this).removeClass('ui-state-hover');
        ...
        viewGridRow: function(rowid, p) {
            ...
            return this.each(function() {
                ...
                function createData(rowid, obj, tb, maxcols) {
                    let nm, hc, trdata, cnt=0, tmp, dc, retpos=[], ind=false, i,
				...
				// find max number align rigth with property formatter
					...
					// hidden fields are included in the form
                        ...
                        if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
                            ...
                            let frmopt = $.extend({}, {rowabove: false, rowcontent: ''}, this.formoptions || {}),
                                rp = parseInt(frmopt.rowpos, 10) || cnt+1,
                                cp = parseInt((parseInt(frmopt.colpos, 10) || 1)*2, 10);
                            if (frmopt.rowabove) {
                                let newdata = $('<tr><td class=\'contentinfo\' colspan=\''+(maxcols*2)+'\'>'+frmopt.rowcontent+'</td></tr>');
                    ...
                    if ( cnt > 0) {
                        let idrow = $('<tr class=\'FormData\' style=\'display:none\'><td class=\'CaptionTD\'></td><td colspan=\''+ (maxcols*2-1)+'\' class=\'DataTD\'><input class=\'FormElement\' id=\'id_g\' type=\'text\' name=\'id\' value=\''+rowid+'\'/></td></tr>');
                        idrow[0].rp = cnt+99;
                        $(tb).append(idrow);
                    }
                    return retpos;
			...
			// set the id.
                ...
                p.gbox = '#gbox_'+$.jgrid.jqID(gID);
                let bt = $('<div></div>').append(frm).append('<table border=\'0\' class=\'EditTable\' id=\''+frmtb+'_2\'><tbody><tr id=\'Act_Buttons\'><td class=\'navButton\' width=\''+p.labelswidth+'\'>'+(rtlb ? bN+bP : bP+bN)+'</td><td class=\'EditButton\'>'+bC+'</td></tr></tbody></table>');
    ...
    $(this).removeClass('ui-state-hover');
        ...
        delGridRow: function(rowids, p) {
            ...
            return this.each(function() {
                ...
                let onBeforeShow = $.isFunction( rp_ge[$t.p.id].beforeShowForm ),
                ...
                if ( $('#'+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
                    ...
                    let dh = isNaN(rp_ge[$t.p.id].dataheight) ? rp_ge[$t.p.id].dataheight : rp_ge[$t.p.id].dataheight+'px',
				...
				// buttons at footer
                    tbl += '</tbody></table></div>';
                    let bS = '<a id=\'dData\' class=\'fm-button ui-state-default ui-corner-all\'>'+p.bSubmit+'</a>',
    ...
    $(this).removeClass('ui-state-hover');
                    ...
                    $('#dData', '#'+dtbl+'_2').click(function() {
                        ...
                        if (ret[0] && !rp_ge[$t.p.id].processing) {
                            ...
                            if (ret[0]) {
                                if (rp_ge[$t.p.id].useDataProxy) {
                                    let dpret = $t.p.dataProxy.call($t, ajaxOptions, 'del_'+$t.p.id);
                                    if (dpret === undefined) {
                                    dpret = [true, ''];
                                }
                                    if (dpret[0] === false ) {
        ...
        navGrid: function(elem, o, pEdit, pAdd, pDel, pSearch, pView) {
            ...
            return this.each(function() {
                ...
                let alertIDs = {themodal: 'alertmod_' + this.p.id, modalhead: 'alerthd_' + this.p.id, modalcontent: 'alertcnt_' + this.p.id},
                    $t = this, twd, tdw;
    ...
    return false;
        ...
        navButtonAdd: function(elem, p) {
            ...
            return this.each(function() {
                ...
                let findnav = $('.navtable', elem)[0], $t = this;
                if (findnav) {
    ...
    $(this).removeClass('ui-state-hover');
        ...
        navSeparatorAdd: function(elem, p) {
            ...
            return this.each(function() {
                ...
                let findnav = $('.navtable', elem)[0];
                if (findnav) {
                    let sep = '<td class=\'ui-pg-button ui-state-disabled\' style=\'width:4px;\'><span class=\''+p.sepclass+'\'></span>'+p.sepcontent+'</td>';
                    if (p.position === 'first') {
                        if (findnav.rows[0].cells.length === 0) {
        ...
        GridToForm: function( rowid, formid ) {
            return this.each(function() {
                ...
                let rowdata = $($t).jqGrid('getRowData', rowid);
                if (rowdata) {
                    for (i in rowdata) {
...
// Editing
		...
		// Compatible mode old versions
            let o={}, args = $.makeArray(arguments).slice(1);
		...
		// End compatible
            return this.each(function() {
                let $t = this, nm, tmp, editable, cnt=0, focus=null, svr={}, ind, cm, bfer;
                if (!$t.grid ) {
                    return;
                }
                ...
                if (editable === '0' && !$(ind).hasClass('not-editable-row')) {
                    ...
                    $('td[role="gridcell"]', ind).each( function(i) {
                        nm = cm[i].name;
                        let treeg = $t.p.treeGrid===true && nm === $t.p.ExpandColumn;
                        if (treeg) {
                        ...
                        if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                            ...
                            if (cm[i].editable===true) {
                                ...
                                let opt = $.extend({}, cm[i].editoptions || {}, {id: rowid+'_'+nm, name: nm, rowId: rowid});
                                if (!cm[i].edittype) {
                                    cm[i].edittype = 'text';
                                }
                                if (tmp === '&nbsp;' || tmp === '&#160;' || (tmp.length===1 && tmp.charCodeAt(0)===160) ) {
                                ...
                                let elc = $.jgrid.createEl.call($t, cm[i].edittype, opt, tmp, true, $.extend({}, $.jgrid.ajaxOptions, $t.p.ajaxSelectOptions || {}));
                    ...
                    if (cnt > 0) {
                        ...
                        if (o.focusField ) {
                            ...
                            setTimeout(function() {
                                let fe = $('td:eq('+focus+') input', ind).not(':disabled');
                                if (fe.length > 0) {
                                    fe.focus();
                                }
                            }, 0);
		...
		// Compatible mode old versions
            let args = $.makeArray(arguments).slice(1), o = {};
		...
		// End compatible
            ...
            let success = false;
            let $t = this[0], nm, tmp={}, tmp2={}, tmp3= {}, editable, fr, cv, ind;
            if (!$t.grid ) {
                return success;
            }
            ind = $($t).jqGrid('getInd', rowid, true);
            ...
            let bfsr = $.isFunction( o.beforeSaveRow ) ?	o.beforeSaveRow.call($t, o, rowid) : undefined;
            if ( bfsr === undefined ) {
                bfsr = true;
            }
            if (!bfsr) {
    ...
    selectedText[i] = $(selected).text();
                ...
                if (cv[0] === false) {
                    try {
                        let tr = $($t).jqGrid('getGridRowById', rowid), positions = $.jgrid.findPos(tr);
                ...
                if (o.url === 'clientArray') {
                    ...
                    let k, resp = $($t).jqGrid('setRowData', rowid, tmp);
                    $(ind).attr('editable', '0');
                    ...
                    $.ajax($.extend({
                        ...
                        error: function(res, stat, err) {
                            ...
                            if ($.isFunction(o.errorfunc) ) {
                                ...
                                let rT = res.responseText || res.statusText;
                                try {
		...
		// End compatible
            ...
            return this.each(function() {
                ...
                let bfcr = $.isFunction( o.beforeCancelRow ) ?	o.beforeCancelRow.call($t, o, sr) : undefined;
                if ( bfcr === undefined ) {
                    bfcr = true;
                }
                if (!bfcr) {
        ...
        addRow: function( p ) {
            ...
            return this.each(function() {
                ...
                let $t = this;
                let bfar = $.isFunction( p.beforeAddRow ) ?	p.beforeAddRow.call($t, p.addRowParams) : undefined;
                if ( bfar === undefined ) {
                    bfar = true;
                }
                if (!bfar) {
                ...
                if (p.useDefValues === true) {
                    $($t.p.colModel).each(function() {
                        if ( this.editoptions && this.editoptions.defaultValue ) {
                            let opt = this.editoptions.defaultValue,
        ...
        inlineNav: function(elem, o) {
            ...
            return this.each(function() {
                ...
                let $t = this, onSelect, gID = $.jgrid.jqID($t.p.id);
                $t.p._inlinenav = true;
			// detect the formatactions column
                ...
                if (o.edit) {
                    $($t).jqGrid('navButtonAdd', elem, {
                        ...
                        id: $t.p.id+'_iledit',
                        onClickButton: function() {
                            let sr = $($t).jqGrid('getGridParam', 'selrow');
                            if (sr) {
                ...
                if (o.cancel) {
                    $($t).jqGrid('navButtonAdd', elem, {
                        ...
                        id: $t.p.id+'_ilcancel',
                        onClickButton: function() {
                            let sr = $t.p.savedRow[0].id, cancelPrm = o.editParams;
                            if (sr) {
                                if ($('#'+$.jgrid.jqID(sr), '#'+gID ).hasClass('jqgrid-new-row')) {
...
* */
    ...
    $.jgrid.extend({
        editCell: function(iRow, iCol, ed) {
			...
			// check.json to see if we have already edited cell
                ...
                if (cm.editable===true && ed===true && !cc.hasClass('not-editable-cell')) {
                    ...
                    if ($.isFunction($t.p.formatCell)) {
                        let tmp2 = $t.p.formatCell.call($t, $t.rows[iRow].id, nm, tmp, iRow, iCol);
                        if (tmp2 !== undefined ) {
                            tmp = tmp2;
                        }
                    }
                    ...
                    let opt = $.extend({}, cm.editoptions || {}, {id: iRow+'_'+nm, name: nm, rowId: $t.rows[iRow].id});
                    let elc = $.jgrid.createEl.call($t, cm.edittype, opt, tmp, true, $.extend({}, $.jgrid.ajaxOptions, $t.p.ajaxSelectOptions || {}));
        ...
        saveCell: function(iRow, iCol) {
            return this.each(function() {
                ...
                if (fr !== null) {
                    let cc = $('td:eq('+iCol+')', $t.rows[iRow]), v, v2,
                    ...
                    case 'select':
                        if (!cm.editoptions.multiple) {
                            ...
                            let sel = $('#'+iRow+'_'+nmjq, $t.rows[iRow]), selectedText = [];
                            v = $(sel).val();
                            if (v) {
                                v.join(',');
                            } else {
    ...
    selectedText[i] = $(selected).text();
				...
				// The common approach is if nothing changed do not do anything
                    if (v2 !== $t.p.savedRow[fr].v) {
                        let vvv = $($t).triggerHandler('jqGridBeforeSaveCell', [$t.rows[iRow].id, nm, v, iRow, iCol]);
                        if (vvv) {
                            v = vvv; v2=vvv;
                        }
                        if ($.isFunction($t.p.beforeSaveCell)) {
                            let vv = $t.p.beforeSaveCell.call($t, $t.rows[iRow].id, nm, v, iRow, iCol);
                            if (vv) {
                                v = vv; v2=vv;
                            }
                        }
                        let cv = $.jgrid.checkValues.call($t, v, iCol);
                        if (cv[0] === true) {
                            let addpost = $($t).triggerHandler('jqGridBeforeSubmitCell', [$t.rows[iRow].id, nm, v, iRow, iCol]) || {};
                            ...
                            if ($t.p.cellsubmit === 'remote') {
                                ...
                                if (stat === 'success') {
                                    let ret = $($t).triggerHandler('jqGridAfterSubmitCell', [$t, result, postdata.id, nm, v, iRow, iCol]) || [true, ''];
        ...
        GridNav: function() {
			...
			// trick to process keydown on non input elements
                $t.p.knv = $t.p.id + '_kn';
                let selection = $('<div style=\'position:fixed;top:0px;width:1px;height:1px;\' tabindex=\'0\'><div tabindex=\'-1\' style=\'width:1px;height:1px;\' id=\''+$t.p.knv+'\'></div></div>'),
                    i, kdir;
                function scrollGrid(iR, iC, tp) {
                    ...
                    if (tp==='h') {
                        let cw = $($t.grid.bDiv)[0].clientWidth,
...
**/
    ...
    $.jgrid.extend({
		...
		// -------------------------
                let subGridCell = function(trdiv, cell, pos)		{
                    let tddiv = $('<td align=\''+ts.p.subGridModel[0].align[pos]+'\'></td>').html(cell);
                    $(trdiv).append(tddiv);
                };
                let subGridXml = function(sjxml, sbid) {
                    let tddiv, i, sgmap,
                    ...
                    if (sjxml) {
                        ...
                        $(sgmap.root+' '+sgmap.row, sjxml).each( function() {
                            ...
                            if (sgmap.repeatitems === true) {
                                ...
                                let f = ts.p.subGridModel[0].mapping || ts.p.subGridModel[0].name;
                                if (f) {
                                    for (i=0; i<f.length; i++) {
                    ...
                    let pID = $('table:first', ts.grid.bDiv).attr('id')+'_';
                    $('#'+$.jgrid.jqID(pID+sbid)).append(dummy);
                    ts.grid.hDiv.loading = false;
                    $('#load_'+$.jgrid.jqID(ts.p.id)).hide();
                    return false;
                };
                let subGridJson = function(sjxml, sbid) {
                    let tddiv, result, i, cur, sgmap, j,
                        dummy = $('<table cellspacing=\'0\' cellpadding=\'0\' border=\'0\'><tbody></tbody></table>'),
                        trdiv = $('<tr></tr>');
                    for (i = 0; i<ts.p.subGridModel[0].name.length; i++) {
                        tddiv = $('<th class=\'ui-state-default ui-th-subgrid ui-th-column ui-th-'+ts.p.direction+'\'></th>');
                ...
                let populatesubgrid = function( rd )		{
                    let sid, dp, i, j;
                    sid = $(rd).attr('id');
                ...
                let _id, pID, atd, nhc=0, bfsc, r;
                $.each(ts.p.colModel, function() {
                    if (this.hidden === true || this.name === 'rn' || this.name === 'cb') {
                        nhc++;
                    }
                });
                let len = ts.rows.length, i=1;
        ...
        expandSubGridRow: function(rowid) {
            return this.each(function() {
                ...
                if ($t.p.subGrid===true) {
                    let rc = $(this).jqGrid('getInd', rowid, true);
                    if (rc) {
                        let sgc = $('td.sgcollapsed', rc)[0];
                        if (sgc) {
                            $(sgc).trigger('click');
                        }
                    }
...
(function($) {
            ...
            return this.each(function() {
				...
				// }
                    ...
                    if (curLevel !== parseInt($t.p.tree_root_level, 10)) {
                        let pn = $($t).jqGrid('getNodeParent', ldat);
    ...
    return false;
        ...
        setTreeGrid: function() {
            return this.each(function() {
                let $t = this, i=0, pico, ecol = false, nm, key, tkey, dupcols=[];
                if (!$t.p.treeGrid) {
                    return;
                }
                if (!$t.p.treedatatype ) {
        ...
        expandRow: function(record) {
            this.each(function() {
                ...
                let childern = $($t).jqGrid('getNodeChildren', record),
			// if ($($t).jqGrid("isVisibleNode",record)) {
                    expanded = $t.p.treeReader.expanded_field;
                $(childern).each(function() {
                    let id = $t.p.idPrefix + $.jgrid.getAccessor(this, $t.p.localReader.id);
        ...
        getFullTreeNode: function(rc) {
            ...
            this.each(function() {
                ...
                case 'adjacency' :
                    if (rc) {
                        result.push(rc);
                        let parent_id = $t.p.treeReader.parent_id_field,
                            dtid = $t.p.localReader.id;
	...
	// End NS, adjacency Model
        getNodeAncestors: function(rc) {
            ...
            this.each(function() {
                ...
                let parent = $(this).jqGrid('getNodeParent', rc);
                while (parent) {
        ...
        isVisibleNode: function(rc) {
            ...
            this.each(function() {
                ...
                let ancestors = $($t).jqGrid('getNodeAncestors', rc),
                    expanded = $t.p.treeReader.expanded_field;
        ...
        isNodeLoaded: function(rc) {
            ...
            this.each(function() {
                ...
                let isLeaf = $t.p.treeReader.leaf_field,
        ...
        expandNode: function(rc) {
            return this.each(function() {
                ...
                let expanded = this.p.treeReader.expanded_field,
                ...
                if (!rc[expanded]) {
                    let id = $.jgrid.getAccessor(rc, this.p.localReader.id);
                    let rc1 = $('#' + this.p.idPrefix + $.jgrid.jqID(id), this.grid.bDiv)[0];
                    let position = this.p._index[id];
        ...
        SortTree: function( sortname, newDir, st, datefmt) {
			...
			// Sorting children
                ...
                $.each(records, function(index) {
                    let id = $.jgrid.getAccessor(this, $t.p.localReader.id);
	...
	// experimental
        ...
        delTreeNode: function(rowid) {
            return this.each(function() {
                let $t = this, rid = $t.p.localReader.id, i,
                    left = $t.p.treeReader.left_field,
                    right = $t.p.treeReader.right_field, myright, width, res, key;
                if (!$t.grid || !$t.p.treeGrid) {
                    return;
                }
                let rc = $t.p._index[rowid];
                if (rc !== undefined) {
				// nested
                    myright = parseInt($t.p.data[rc][right], 10);
                    width = myright - parseInt($t.p.data[rc][left], 10) + 1;
                    let dr = $($t).jqGrid('getFullTreeNode', $t.p.data[rc]);
                    if (dr.length>0) {
		...
		// return this.each(function(){
			...
			// we suppose tha the id is autoincremet and
                let expanded = $t.p.treeReader.expanded_field,
                    isLeaf = $t.p.treeReader.leaf_field,
                    level = $t.p.treeReader.level_field,
			// icon = $t.p.treeReader.icon_field,
                ...
                if ( nodeid === undefined || nodeid === null ) {
                    ...
                    if (	i>= 0 ) {
                        while (i>=0) {
                            max = Math.max(max, parseInt($t.p.data[i][$t.p.localReader.id], 10)); i--;
                        }
                    }
                    nodeid = max+1;
                }
                let prow = $($t).jqGrid('getInd', parentid);
                leaf = false;
			// if not a parent we assume root
                if ( parentid === undefined || parentid === null || parentid==='') {
                    ...
                    let childs = $($t).jqGrid('getFullTreeNode', parentdata);
...
(function($) {
    ...
    $.extend($.jgrid, {
        template: function(format) { // jqgformat
            let args = $.makeArray(arguments).slice(1), j, al = args.length;
            if (format==null) {
                format = '';
            }
    ...
    $.jgrid.extend({
        groupingSetup: function() {
            return this.each(function() {
                let $t = this, i, j, cml, cm = $t.p.colModel, grp = $t.p.groupingView;
        ...
        groupingPrepare: function( record, irow ) {
            this.each(function() {
                let grp = this.p.groupingView, $t= this, i,
            ...
            this.each(function() {
                ...
                let uid = strpos.join('_'),
                    ...
                    getGroupingLevelFromClass = function(className) {
                        let nums = $.map(className.split(' '), function(item) {
                        ...
                        if (r) {
                            while (r) {
                                if ($(r).hasClass('jqfoot') ) {
                                    let lv = parseInt($(r).attr('jqfootlevel'), 10);
                                    if ( lv <= num) {
                                    break;
                                }
                                }
        ...
        groupingRender: function(grdata, colspans, page, rn ) {
            return this.each(function() {
                ...
                function buildSummaryTd(i, ik, grp, foffset) {
                    let fdata = findGroupIdx(i, ik, grp),
                        cm = $t.p.colModel,
                ...
                let sumreverse = $.makeArray(grp.groupSummary), mul;
                sumreverse.reverse();
                ...
                $.each(grp.groups, function(i, n) {
                    ...
                    let leaf = len-1 === n.idx;
                    if ( leaf ) {
                        let gg = grp.groups[i+1], kk, ik, offset = 0, sgr = n.startRow,
...
**/
    ...
    $.jgrid.extend({
        jqGridImport: function(o) {
            ...
            return this.each(function() {
                let $t = this;
                let xmlConvert = function(xml, o) {
                    let cnfg = $(o.xmlGrid.config, xml)[0];
                    let xmldata = $(o.xmlGrid.data, xml)[0], jstr, jstr1, key;
                    if (xmlJsonClass.xml2json && $.jgrid.parse) {
                        ...
                        if (xmldata) {
                        // save the datatype
                            let svdatatype = jstr.grid.datatype;
                ...
                let jsonConvert = function(jsonstr, o) {
                    if (jsonstr && typeof jsonstr === 'string') {
                        let _jsonparse = false;
                        if ($.jgrid.useJSON) {
                            $.jgrid.useJSON = false;
                            _jsonparse = true;
                        }
                        let json = $.jgrid.parse(jsonstr);
                        if (_jsonparse) {
                            $.jgrid.useJSON = true;
                        }
                        let gprm = json[o.jsonGrid.config];
                        let jdata = json[o.jsonGrid.data];
                        if (jdata) {
                            let svdatatype = gprm.datatype;
        ...
        jqGridExport: function(o) {
            ...
            this.each(function() {
                ...
                let key, gprm = $.extend(true, {}, $(this).jqGrid('getGridParam'));
                // we need to check.json for:
        ...
        excelExport: function(o) {
            ...
            return this.each(function() {
                ...
                let url;
                if (o.exptype === 'remote') {
                    let pdata = $.extend({}, this.p.postData);
                    pdata[o.oper] = o.tag;
                    let params = jQuery.param(pdata);
                    if (o.url.indexOf('?') !== -1) {
                        url = o.url+'&'+params;
                    } else {
                        url = o.url+'?'+params;
...
// requiere load multiselect before grid
    ...
    if ($.ui) {
        if ($.ui.multiselect ) {
            if ($.ui.multiselect.prototype._setSelected) {
                let setSelected = $.ui.multiselect.prototype._setSelected;
                $.ui.multiselect.prototype._setSelected = function(item, selected) {
                    let ret = setSelected.call(this, item, selected);
    ...
    $.jgrid.extend({
        sortableColumns: function(tblrow)	{
            return this.each(function() {
                let ts = this, tid= $.jgrid.jqID( ts.p.id );
                function start() {
                ...
                let sortable_opts = {
                    ...
                    'placeholder': {
                        element: function(item) {
                            let el = $(document.createElement(item[0].nodeName))
                    ...
                    'update': function(event, ui) {
                        ...
                        let permutation = [];
                        th.each(function() {
                            let id = $('>div', this).get(0).id.replace(/^jqgh_/, '').replace(tid, '');
        ...
        columnChooser: function(opts) {
            ...
            let selector = $('<div id="colchooser_'+self[0].p.id+'" style="position:relative;overflow:hidden"><div><select multiple="multiple"></select></div></div>');
            let select = $('select', selector);

            function insert(perm, i, v) {
                if (i>=0) {
                    let a = perm.slice();
                    let b = a.splice(i, Math.max(perm.length-i, i));
                    if (i>perm.length) {
                        i = perm.length;
                    }
                    a[i] = v;
            ...
            let colNames = self.jqGrid('getGridParam', 'colNames');
            ...
            let dopts = $.isFunction(opts.dlog_opts) ? opts.dlog_opts.call(self, opts) : opts.dlog_opts;
            call(opts.dlog, selector, dopts);
            let mopts = $.isFunction(opts.msel_opts) ? opts.msel_opts.call(self, opts) : opts.msel_opts;
		...
		// Currently we disable a treeGrid drag and drop
                ...
                function updateDnD()		{
                    let datadnd = $.data($t, 'dnd');
                    $('tr.jqgrow:not(.ui-draggable)', $t).draggable($.isFunction(datadnd.drag) ? datadnd.drag.call($($t), datadnd) : datadnd.drag);
                }
                let appender = '<table id=\'jqgrid_dnd\' class=\'ui-jqgrid-dnd\'></table>';
                    ...
                    'drop': function(opts) {
                        return $.extend({
                            accept: function(d) {
                                ...
                                let tid = $(d).closest('table.ui-jqgrid-btable');
                                if (tid.length > 0 && $.data(tid[0], 'dnd') !== undefined) {
                                    let cn = $.data(tid[0], 'dnd').connectWith;
                            ...
                            drop: function(ev, ui) {
                                ...
                                let accept = $(ui.draggable).attr('id');
                                let getdata = ui.draggable.parent().parent().jqGrid('getRowData', accept);
                                if (!opts.dropbyname) {
                                    let j =0, tmpdata = {}, nm, key;
                                    let dropmodel = $('#'+$.jgrid.jqID(this.id)).jqGrid('getGridParam', 'colModel');
                                    try {
                                    for (key in getdata) {
							...
							// should return object which will be inserted into the reciever
                                    let datatoinsert = opts.beforedrop.call(this, ev, ui, getdata, $('#'+$.jgrid.jqID($t.p.id)), $(this));
        ...
        gridResize: function(opts) {
            return this.each(function() {
                let $t = this, gID = $.jgrid.jqID($t.p.id);
                ...
                if (opts._alsoResize_) {
                    let optstest = '{\'#gview_'+gID+' .ui-jqgrid-bdiv\':true,\'' +opts._alsoResize_+'\':true}';
...
function tableToGrid(selector, options) {
	...
	// Text whether we have single or multi select
        let inputCheckbox = jQuery('tr td:first-child input[type=checkbox]:first', jQuery(this));
        let inputRadio = jQuery('tr td:first-child input[type=radio]:first', jQuery(this));
        let selectMultiple = inputCheckbox.length > 0;
        let selectSingle = !selectMultiple && inputRadio.length > 0;
        let selectable = selectMultiple || selectSingle;
        ...
        jQuery('tbody > tr', jQuery(this)).each(function() {
            ...
            jQuery('td', jQuery(this)).each(function() {
                if (rowPos === 0 && selectable) {
                    let input = jQuery('input', jQuery(this));
                    let rowId = input.attr('value');
    ...
    $.jgrid.extend({
		...
		// again with group setup options which then will be passed to the grid
			 ...
			 */
				...
				// default is sum
                    let arrln = aggr.length, i, label, j, jv;
                    if ($.isArray(value)) {
                        jv = value.length;
                    } else {
                        jv = 1;
			...
			// Making the row totals without to add in yDimension
                if (o.rowTotals && o.yDimension.length > 0) {
                    let dn = o.yDimension[0].dataName;
			...
			// loop over alll the source data
				...
				// build the data from xDimension
                    ...
                    let k = 0;
                    rowindex = -1;
				// check.json to see if the row is in our new pivotrow set
                ...
                let lastval=[], initColLen = columns.length, swaplen = initColLen;
                if (ylen>0) {
					...
					// and write name and newline
							...
							// If not a object build the header of the appropriate level
                                if ( key === 'level') {
                                    ...
                                    let collen = headers[items.level-1].groupHeaders.length,
        ...
        jqPivot: function( data, pivotOpt, gridOpt, ajaxOpt) {
            return this.each(function() {
                ...
                function pivot( data) {
                    let pivotGrid = jQuery($t).jqGrid('pivotSetup', data, pivotOpt),
                    ...
                    let gHead = pivotGrid.groupHeaders;
                    if (gHead.length) {
