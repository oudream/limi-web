...
// This is the ols search Filter method used in navigator.
            ...
            return this.each(function() {
                ...
                */
                ...
                // Pulling default filter settings out of postData property of grid's properties.:
                    let defaultFilters = gridDOMobj.p.postData[filterSettings.sFilter];
				...
				// suppose we have imported this with grid import, the this is a string.
                    ...
                    if (defaultFilters) {
                        ...
                        if (defaultFilters.rules) {
                            let f, i = 0, li = defaultFilters.rules.length, success = false;
                            for (; i < li; i++) {
                ...
                function showFilter() {
                    let fl = $('.ui-searchFilter').length;
                    if (fl > 1) {
                        let zI = $('#'+fid).css('zIndex');
                ...
                function searchFilters(filters) {
                    let hasFilters = (filters !== undefined),
                        grid = $('#'+$t.p.id),
                        sdata={};
                ...
                function resetFilters(op) {
                    let reload = op && op.hasOwnProperty('reload') ? op.reload : true,
                        grid = $('#'+$t.p.id),
                        sdata={};
                ...
                if ($.fn.searchFilter) {
				    ...
				    $.each(colModel, function(i, v) {
				        let searchable = (typeof v.search === 'undefined') ? true: v.search,
        ...
        if (this.stype == 'select') {
            if ( soptions.dataUrl !== undefined) {}							else {
                let eov;
                if (soptions.value) {
                    eov = soptions.value;
	...
	// methods taken from grid.custom.
        updateGridRows: function(data, rowidname, jsonreader) {
            let nm, success=false, title;
            this.each(function() {
                let t = this, vl, ind, srow, sid;
                if (!t.grid) {
                    return false;
                }
                if (!rowidname) {
	...
	// Form search - sorry for this method. Instead use ne jqFilter method.
        filterGrid: function(gridid, p) {
            ...
            return this.each(function() {
                ...
                let gcolMod = $(this.p.gridid).jqGrid('getGridParam', 'colModel');
                if (gcolMod) {
                    if ( this.p.gridModel === true) {
                        let thegrid = $(this.p.gridid)[0];
                        let sh;
					...
					// additionally surl and defval can be added in grid colModel
                        ...
                        $.each(self.p.filterModel, function(i, n) {
                            for (let j=0; j<gcolMod.length; j++) {
                ...
                let frm = $('<form name=\'SearchForm\' style=display:inline;\' class=\''+this.p.formclass+'\'></form>');
